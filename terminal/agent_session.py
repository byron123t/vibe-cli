...
class AgentSession(ABC):
    ...
    def __init__(
        ...
        self.max_turns          = max_turns         # limit agentic turns (Claude)
        self.max_budget_usd     = max_budget_usd    # USD spending cap (Claude)
        self.system_prompt      = system_prompt     # appended to system prompt (Claude)
        self.allowed_tools      = allowed_tools or []    # tool patterns that skip approval
        self.disallowed_tools   = disallowed_tools or [] # tool patterns removed from context
        ...
        self.verbose_output: bool = verbose_output  # emit full tool inputs + results
        self.output_tokens:  int  = 0               # tokens generated (populated by subclasses)
        self.input_tokens:   int  = 0               # tokens sent (populated by subclasses)
        self.active_model:   str  = ""              # model name (populated by subclasses)
